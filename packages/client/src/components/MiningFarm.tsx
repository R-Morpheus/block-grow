...
const MiningFarm = () => {
  ...
  const token = useComponentValue(Token, playerEntity)?.name
  const entities = useEntityQuery([Has(Token)])
  console.log(entities)
  const entity = entities[0]

  console.log(getComponentValueStrict(Token, entity))


  const valueList = [
    {nameData: 'Level', valueData: useComponentValue(MiningLevel, playerEntity)?.value},
    {nameData: 'Token', valueData: useComponentValue(Token, playerEntity)?.name,},
    {nameData: 'Work', valueData: useComponentValue(MiningWork, playerEntity)?.work},
  ...
  return (
    <div>
      <button onClick={() => console.log('token name', token)}>
        logger
      </button>
